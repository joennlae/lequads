var tutorialLayer = cc.LayerColor.extend({
    ...
    init: function() {
        if (this.ls.getItem(666) == 2 || this.ls.getItem(666) == 3) { //because native jsb only allow init()in ctor 1,5 hours :-)
            ...
            var backItemLabel = new cc.MenuItemSprite(new cc.Sprite(res.forward), new cc.Sprite(res.forward), this.onRestart, this);
            ...
            for (i = littlelvl.length - 1; i > 0; i--) {
                for (j = 0; j < littlelvl[0].length; j++) {
                    if (littlelvl[i][j] !== 0) {
                        ...
                        sprite.attr({
                            x: (j * this.sizeOfSprite + winSize.width / 25),
                            y: ((littlelvl.length - i) * this.sizeOfSprite),
    ...
    initTwo: function() {
        ...
        this.scheduleUpdate();
        var scalerY = winSize.height / 1080;
        var backItemLabel = new cc.MenuItemSprite(new cc.Sprite(res.forward), new cc.Sprite(res.forward), this.onRestartTwo, this);
        backItemLabel.scale = scalerY;
        var backMenu = new cc.Menu(backItemLabel);
        backMenu.setPosition(cc.p(winSize.width / 8 * 7.5, winSize.height / 8));
        ...
        if (this.leswitcher.gameModeThree == 0) { //color Red
            switch (this.level[railPosY][railPosX]) {
                ...
                case 35:
                    ...
                    var sprite = new cc.Sprite(spriteFrame);
                    sprite.attr({
                        x: (railPosX * this.sizeOfSprite + winSize.width / 25),
                        y: ((this.level.length - railPosY) * this.sizeOfSprite),
                        scale: this.scaleFactor
                    });
            ...
            switch (lm) { //14 when circle, 12 when destroyed with last move
                ...
                case 3:
                    if (this.level[pY - 1][pX] + this.level[pY - 1][pX - 1] + this.level[pY][pX - 1] + this.level[pY][pX] == 14) {
                        ...
                        var sprite = new cc.Sprite(spriteFrame);
                        sprite.attr({
                            x: ((pX - 0.5) * this.sizeOfSprite + winSize.width / 25),
                            y: ((this.level.length - pY + 0.5) * this.sizeOfSprite),
                            scale: this.scaleFactor
                        });
